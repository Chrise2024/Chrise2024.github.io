...
1. `float(x:any) -> float` 将x转换为浮点类型

1. `complex(real:number [,imag:number]) -> complex` 创建一个复数

> 可选参数：`imag`，指定虚部，默认为0

1. `bool(x:any) -> bool` 将x转换为布尔

> 任意不为0的数字或不为空的集合/字符串将被转换为`True`，反之则为`False`

1. `str(x:any) -> str` 将x转换为字符串

1. `tuple(s:iterable) -> tuple` 将s转换为元组，不提供s则创建空元组

1. `list(s:iterable) -> list` 将s转换为列表，不提供s则创建空列表

1. `set(s:iterable) -> set` 将s转换为集合，不提供s则创建空集合

1. `dict(s:iterable) -> dict` 将s转换为字典，不提供s则创建空字典

1. `frozenset(s:iterable) -> any` 将s转换为不可变集合

1. `chr(x:int) -> str` 将整数x转换为对应编码的字符

1. `ord(x:str) -> int` 将字符x转换为对应的编码

1. `hex(x:int) -> str` 将整数x转换为十六进制字符串

1. `oct(x:int) -> str` 将整数x转换为八进制字符串

1. `bin(x:int) -> str` 将整数x转换为二进制字符串

1. `eval(x:str) -> any` 将字符串解析为`Python`表达式
